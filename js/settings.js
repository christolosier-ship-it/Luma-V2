const SettingsScreen = {
  render() {
    ...
    screen.innerHTML = `<div class="section-title">Réglages</div><div class="settings-section card">
      ...
      <div class="settings-row-help">Format de sauvegarde local Luma.</div><input type="file" id="import-file-input" accept=".json" class="hidden" /></div><div class="settings-credits">Luma V3.4.1 · Créé par Christopher Losier</div>`;
    screen.querySelector('#btn-export').addEventListener('click', async () => {
      try { const data = await DB.exportAll(); SettingsScreen._downloadJson(data, `luma-v3.4.1-backup-${todayStr()}.json`); showToast('✓ Export JSON réalisé'); }
