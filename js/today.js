  ...
  async render() {
    ...
    try {
      ...
      const symptomFields = [
        ...
        { key:'other', label: daySymptoms.otherSymptomLabel || 'Autre ressenti' }
      ];
      ...
      <details class='card card-sm' ${hasSymptomsData?'open':''}><summary><strong>Ressentis du jour</strong>${hasSymptomsData?`<span class='today-count' style='display:block;'>${escHtml(symptomsSummary)}</span>`:''}</summary><div class='symptom-grid' style='margin-top:8px;'>${symptomSelects}</div><label style='display:block;margin-top:8px;'>Libellé autre ressenti<input id='n-other-label' class='form-input' value='${escHtml(daySymptoms.otherSymptomLabel||'')}' placeholder='Ex. fourmillements'></label><button id='save-symptoms' class='btn btn-primary btn-compact' style='margin-top:8px;'>Enregistrer les ressentis</button></details>`;
      ...
      screen.querySelectorAll('.btn-toggle-event').forEach(btn=>btn.addEventListener('click', async ()=>{await DB.toggleProtocolEventCompleted(btn.dataset.id); await window.LumaNotifications?.refreshAfterDataChange?.(); await TodayScreen.render(); await TimelineScreen.render();}));
