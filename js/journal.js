const JournalScreen = {
  ...
  _journalIntakeLine(i,date){ return `<div class='journal-line'><div>${escHtml(i.displayTime || i.time || 'Sans horaire')} · ${escHtml(i.medName)}<div class='timeline-item-detail'>${escHtml(i.dosage || '')}${i.dosageMode === 'variable' ? ' · Dosage variable' : ''}${i.historical ? ' · Traitement modifié depuis' : ''}</div></div><div><span class='timeline-badge'>${escHtml(i.historical ? 'Historique' : this._statusBadge(i.status,date))}</span> · ${escHtml(this._delayText(i))}</div></div>`; },
  ...
  _delayText(i){ if(i.status!=='taken' || !i.takenAt) return 'non renseigné'; const [h,m]=String(i.time || '00:00').split(':').map(Number); const d=new Date(i.takenAt); const delay=(d.getHours()*60+d.getMinutes())-(h*60+m); if(delay===0) return "à l'heure"; return delay>0?`+${delay} min`:`${delay} min`; },
  _historicalIntakeFromEvent(ev){ const payload = ev.payload || {}; const parts = (ev.intakeKey || '').split('|'); const scheduledDate = payload.scheduledDate || parts[2]; const scheduledTime = payload.scheduledTime || parts[3] || ''; return { key: ev.intakeKey || `${ev.medicationId || 'historique'}|historique|${scheduledDate}|${scheduledTime}`, medId: ev.medicationId || '', protocolId: ev.protocolId || '', dateStr: scheduledDate, time: scheduledTime, displayTime: scheduledTime, medName: payload.medNameSnapshot || 'Traitement supprimé ou modifié', dosage: payload.dosageSnapshot || payload.plannedDosage || '', dosageMode: payload.dosageModeSnapshot || 'fixed', status: ev.type === 'skipped' ? 'skipped' : ev.type === 'snoozed' ? 'snoozed' : 'taken', takenAt: ev.type === 'taken' ? ev.createdAt : null, historical: true }; },
  _applySnapshotIfNeeded(intake, ev){
    if (!ev || !['taken','skipped','snoozed'].includes(ev.type)) return intake;
  ...
  _buildDays(notes, symptomsEntries, meds, phases, dosageOverrides, actions, protocolEvents, protocols, intakeEvents){
    ...
    if(this.period==='custom'&&this.customStart&&this.customEnd&&this.customEnd>=this.customStart){let d=new Date(this.customEnd);const s=this.customStart;while(toDateStr(d)>=s){dates.push(toDateStr(d));d.setDate(d.getDate()-1);}}
    ...
    for(const ds of dates){
      ...
      const historical = [...historyByKey.values()].filter(e=>['taken','skipped','snoozed'].includes(e.type)).map(e=>this._historicalIntakeFromEvent(e));
  ...
  _stats(days){ const now=new Date(); const today=todayStr(); const flat=[]; for(const d of days){ for(const i of d.intakes){ if(d.date>today) continue; if(d.date===today){const [h,m]=String(i.displayTime || i.time || '99:99').split(':').map(Number); if((h*60+m)>(now.getHours()*60+now.getMinutes())&&i.status==='pending') continue;} flat.push({...i,date:d.date}); } } const planned=flat.length; const taken=flat.filter(i=>i.status==='taken').length; const skipped=flat.filter(i=>i.status==='skipped').length; const snoozed=flat.filter(i=>i.status==='snoozed').length; const missed=flat.filter(i=>i.status==='pending').length; const delays=flat.filter(i=>i.status==='taken'&&i.takenAt).map(i=>{const [h,m]=String(i.time || '00:00').split(':').map(Number); const d=new Date(i.takenAt); return Math.max(0,d.getHours()*60+d.getMinutes()-(h*60+m));}); return {planned,taken,skipped,snoozed,missed,adherence:planned?Math.round((taken/planned)*100):0,avgDelay:Math.round(delays.reduce((a,b)=>a+b,0)/Math.max(1,delays.length)),eventsPlanned:days.flatMap(d=>d.events).length,eventsDone:days.flatMap(d=>d.events).filter(e=>e.completed).length};},
  _csvLineForItem(d, item, protocols){
    if(item.kind==='intake'){ const i=item.data; const p=protocols.find(pr=>pr.id===i.protocolId)||{}; return [d.date,p.name||'', 'prise',i.displayTime || i.time || '',i.takenAt?new Date(i.takenAt).toTimeString().slice(0,5):'',i.medName,i.dosage,statusLabelFR(i.status),'','', '', i.historical ? 'Historique - traitement modifié depuis' : (i.dosageMode === 'variable' ? 'Dosage variable' : 'Traitement simple')]; }
