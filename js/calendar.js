...
const CalendarScreen = {
  ...
  _navHtml() {
    ...
    return `
      <div class="cal-nav">
        ...
        <div class="cal-month">${capitalize(monthName)}</div>
  ...
  _gridHtml(selectedDateStr, datesWithIntakes) {
    ...
    for (let i = 0; i < startDow; i++) {
      ...
      cells += `<div class="cal-day other-month" data-date="${dateStr}">${day}</div>`;
    }
    ...
    for (let i = 1; i <= remaining; i++) {
      ...
      cells += `<div class="cal-day other-month" data-date="${dateStr}">${i}</div>`;
    }
