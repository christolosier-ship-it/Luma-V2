const TodayScreen = {
  async render() {
    ...
    try {
      ...
      const missingVariableNotice = missingVariableMeds.length ? `<div class='card card-sm today-info-notice'><p>Un traitement à dosage variable est actif aujourd’hui, mais aucun dosage n’est renseigné.</p><p>${missingVariableMeds.map(m=>escHtml(m.name)).join(' · ')}</p><button class='btn-settings js-open-missing-dosage' data-med-id='${escHtml(missingVariableMeds[0].id)}'>Ouvrir le calendrier de dosage</button></div>` : '';
      ...
      screen.querySelector('.js-open-missing-dosage')?.addEventListener('click', async (e)=>{ const med = medications.find(m => m.id === e.currentTarget.dataset.medId); if (med) await MedicationsScreen.openDosageCalendar(med, dateStr); });
  ...
  _getNextAction(intakes, dayProtocolEvents, nowMinutes){
    ...
    intakes.forEach(i=>{
      ...
      candidates.push({type:'intake', title:i.medName, timeLabel:sourceTime||'Sans horaire', status:i.status, minutes, hasTime});
    });
    dayProtocolEvents.forEach(e=>{
      ...
      candidates.push({type:'event', title:e.title, timeLabel:e.time||'Sans horaire', status:e.completed?'completed':'pending', minutes, hasTime});
    });
