const TodayScreen = {
  async render() {
    ...
    try {
      ...
      screen.querySelector('#btn-save-note')?.addEventListener('click', async()=>{ const freeNote = document.getElementById('n-free').value.trim(); if(!freeNote){ await DB.deleteDailyNote(dateStr); showToast('Note supprimée'); } else { const note={id:dateStr,date:dateStr,freeNote,updatedAt:new Date().toISOString(),createdAt:dayNote.createdAt||new Date().toISOString()}; await DB.saveDailyNote(note); showToast('Note enregistrée'); } await TodayScreen.render(); await TimelineScreen.render(); await JournalScreen.render(); });
  ...
  _getNextAction(intakes, dayProtocolEvents, nowMinutes){
    const parseMinutes = (time) => /^\d{2}:\d{2}$/.test(time || '') ? time.split(':').map(Number).reduce((a,v,idx)=>a+(idx===0?v*60:v),0) : Number.POSITIVE_INFINITY;
    const candidates = [];
    intakes.forEach(i=>{
      if(i.status==='taken' || i.status==='skipped') return;
      const sourceTime = i.displayTime || i.time || '';
      const hasTime = /^\d{2}:\d{2}$/.test(sourceTime);
      candidates.push({type:'intake', title:i.medName || '', timeLabel:sourceTime||'Sans horaire', status:i.status, minutes:parseMinutes(sourceTime), hasTime});
    });
    dayProtocolEvents.forEach(e=>{
      if(e.completed) return;
      const hasTime = /^\d{2}:\d{2}$/.test(e.time||'');
      candidates.push({type:'event', title:e.title || '', timeLabel:e.time||'Sans horaire', status:'pending', minutes:parseMinutes(e.time), hasTime});
    });
    if(!candidates.length) return null;
    const byTime = (a,b)=>(a.minutes-b.minutes)||String(a.title).localeCompare(String(b.title));
    const late = candidates.filter(c=>c.hasTime && c.minutes < nowMinutes).sort(byTime)[0];
    if (late) return { ...late, prefix: 'Action en retard' };
    const future = candidates.filter(c=>c.hasTime && c.minutes >= nowMinutes).sort(byTime)[0];
    if (future) return { ...future, prefix: 'Prochaine action' };
    const untimed = candidates.filter(c=>!c.hasTime).sort((a,b)=>String(a.title).localeCompare(String(b.title)))[0];
    return untimed ? { ...untimed, prefix: 'Prochaine action' } : null;
  },
