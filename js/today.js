const TodayScreen = {
  async render() {
    ...
    try {
      ...
      screen.querySelector('#btn-save-note')?.addEventListener('click', async()=>{ const freeNote = document.getElementById('n-free').value.trim(); const note={id:dateStr,date:dateStr,freeNote,updatedAt:new Date().toISOString(),createdAt:dayNote.createdAt||new Date().toISOString()}; await DB.saveDailyNote(note); showToast('Note enregistrée'); await JournalScreen.render(); await TimelineScreen.render(); });
