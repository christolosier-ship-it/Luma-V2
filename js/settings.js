const SettingsScreen = {
  render() {
    ...
    screen.innerHTML = `<div class="section-title">Réglages</div><div class="settings-section card">
      <div class="settings-row"><div><div class="settings-row-label">Exporter JSON V3.4</div></div><button class="btn-settings" id="btn-export">Exporter</button></div>
      <div class="settings-row"><div><div class="settings-row-label">Importer JSON (V2/V2.1/V3/V3.4)</div></div><button class="btn-settings" id="btn-import">Importer</button></div>
    ...
    screen.querySelector('#btn-export').addEventListener('click', async () => {
      try { const data = await DB.exportAll(); SettingsScreen._downloadJson(data, `luma-v3.4-backup-${todayStr()}.json`); showToast('✓ Export JSON réalisé'); }
